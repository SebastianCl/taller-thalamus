...
export const designDocumentSchema = z
  .object({
    ...
    layers: z
      .array(z.discriminatedUnion('type', [imageLayerSchema, textLayerSchema]))
      .max(20),
    ...
    for (const zone of GARMENT_ZONES[document.modelId]) {
      if (!document.zones[zone])
        context.addIssue({
          code: 'custom',
          path: ['zones', zone],
          message: 'Falta una zona de la prenda.',
        });
    }
    const ids = new Set<string>();
    const orders = new Set<number>();
    document.layers.forEach((layer, index) => {
      if (!document.zones[layer.zone])
        context.addIssue({
          code: 'custom',
          path: ['layers', index, 'zone'],
          message: 'La capa no tiene una zona guardada.',
        });
      if (ids.has(layer.id))
        context.addIssue({
          code: 'custom',
          path: ['layers', index, 'id'],
          message: 'El identificador de capa está duplicado.',
        });
      if (orders.has(layer.order))
        context.addIssue({
          code: 'custom',
          path: ['layers', index, 'order'],
          message: 'El orden de capa está duplicado.',
        });
      ids.add(layer.id);
    ...
    for (let order = 0; order < document.layers.length; order += 1) {
      if (!orders.has(order))
        context.addIssue({
          code: 'custom',
          path: ['layers'],
          message: 'El orden de las capas debe ser continuo.',
        });
    }
  });

export const webMcpTemplateInputSchema = z
  .object({ templateId: z.enum(templateIds) })
  .strict();
export const webMcpColorInputSchema = z
  .object({ zone: zoneIdSchema, color: z.string().regex(/^#[0-9a-f]{6}$/i) })
  .strict();
export const webMcpTextInputSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (value.subtype === 'number' && !/^\d{1,3}$/.test(value.text)) {
      context.addIssue({
        code: 'custom',
        path: ['text'],
        message: 'Los números deben contener de 1 a 3 dígitos.',
      });
    }
  });
...
export function parseDesignDocument(input: unknown): DesignDocument {
  // The first V2 hoodie had no pocket. Extend only that known legacy shape;
  // all other missing zones and all imported styles still require validation.
  if (
    isRecord(input) &&
    input.schemaVersion === 2 &&
    input.modelId === 'taller-hoodie-v1' &&
    isRecord(input.zones) &&
    !Object.hasOwn(input.zones, 'pocket')
  ) {
    const front = zoneStyleSchema.parse(input.zones.front);
    input = {
      ...input,
      zones: {
        ...input.zones,
        pocket: {
          ...createDocument('taller-hoodie-v1').zones.pocket,
          color: front.color,
        },
      },
    };
  ...
  if (!isRecord(input) || input.schemaVersion !== 0)
    return designDocumentSchema.parse(input) as DesignDocument;
  ...
  const zones = Object.fromEntries(
    ZONE_IDS.map((zone) => [
      ...
      {
        ...fallback.zones[zone],
        ...(isRecord(legacyZones[zone]) ? legacyZones[zone] : {}),
      },
    ]),
  );
  const layers = Array.isArray(input.layers)
    ? input.layers.map((layer, order) =>
        isRecord(layer)
          ? { visible: true, locked: false, ...layer, order }
          : layer,
      )
    : [];
  return designDocumentSchema.parse({
    ...
    templateId: templateIds.includes(
      input.templateId as (typeof templateIds)[number],
    )
      ? input.templateId
      : 'blank',
    zones,
