...
/* oxlint-disable jsx-a11y/no-noninteractive-tabindex -- The 3D application surface accepts keyboard editing and must be focusable. */
...
import {
  loadUvZoneMask,
  zoneFromUvMask,
  type UvZoneMaskLookup,
} from '@/lib/uv-zone-mask';
import { ZONE_LABELS, type ViewId, type ZoneId } from '@/lib/design';
import { cn } from '@/lib/utils';
import { useEditorStore } from '@/store/editor-store';

type CapturedViews = Record<ViewId, Blob>;
function currentGarment() {
  return getGarment(useEditorStore.getState().document.modelId);
}
...
function atlasRectFor(
  zone: ZoneId,
  zoneMask?: UvZoneMaskLookup | null,
): PixelRect {
  const rect =
    zoneMask?.rects?.[zone]?.[0] ??
    currentGarment().manifest.atlas.zones[zone].rect;
  return rect;
}

function imageAspectFor(
  layer: { type: string; assetId?: string },
...
function layerHandleAtPointer(
  ...
  const frame = getLayerFrame(
    ...
    rect,
    layer,
    imageAspectFor(layer, assets),
  );
  const point = pointInAtlasRect(zone, uv, zoneMask);
  return hitTestLayerHandle(
    frame,
    point.x,
    point.y,
    layer.transform.rotation,
    Math.max(0.02, rect.width * 0.035),
  );
}
...
function resizeFromPoint(
  ...
  const ratio = Math.max(
    0.01,
    (localX * startDx + localY * startDy) / denominator,
  );
...
function viewRotation(view: ViewId) {
  return currentGarment().manifest.cameras[view].rotationY;
}

function useAtlasTexture(masked = true) {
  const garment = currentGarment();
  const [maskError, setMaskError] = useState<Error | null>(null);
  ...
  const canvas = useMemo(() => {
    ...
    element.width = element.height = mobile
      ? currentGarment().manifest.atlas.mobileSize
      : currentGarment().manifest.atlas.desktopSize;
    return element;
  }, []);
  ...
  useEffect(() => {
    ...
    void loadUvZoneMask(canvas.width, controller.signal, garment.manifest)
      .then(setZoneMask)
      .catch((error: unknown) => {
        if (!(error instanceof DOMException && error.name === 'AbortError')) {
          console.error(
            'No se pudo aplicar la máscara UV exacta; se usará la segmentación de respaldo.',
            error,
          );
        }
      });
    return () => controller.abort();
  }, [canvas, masked, garment]);
  ...
  useEffect(() => {
    ...
    for (const asset of Object.values(assets)) {
      ...
      image.onerror = () => {
        if (!disposed)
          setMaskError(new Error('No se pudo cargar una imagen del diseño.'));
      };
  ...
  useEffect(() => {
    ...
    void window.document.fonts.ready.then(() => {
      if (active) setFontVersion((version) => version + 1);
    });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    const visibleSelection =
      interactionMode === 'move' ? selectedLayerId : null;
    renderAtlas(
      canvas,
      document,
      assets,
    ...
    const ready =
      ...
      Object.values(assets).every((asset) =>
        Boolean(imageCache.current[asset.id]),
      );
    const state = useEditorStore.getState();
    if (state.stageStatus !== 'error') {
      state.setStageStatus(garment.id, ready ? 'ready' : 'loading');
    }
  }, [
    assets,
    canvas,
...
function createFabricScalarTexture(base: number, variation: number) {
  ...
  for (let index = 0; index < 64 * 64; index += 1) {
    ...
    const value = Math.max(
      0,
      Math.min(255, Math.round(base + (seed / 2147483647 - 0.5) * variation)),
    );
    data[index * 4] = value;
...
function LicensedJerseyModel({
  groupRef,
}: {
  groupRef: RefObject<THREE.Group | null>;
}) {
  const gltf = useGLTF(currentGarment().url) as unknown as {
    scene: THREE.Group;
  };
  const invalidate = useThree((state) => state.invalidate);
  ...
  const updateLayerResizeLive = useEditorStore(
    (state) => state.updateLayerResizeLive,
  );
  ...
  const model = useMemo(() => {
    ...
    // Frame relaxed long sleeves in narrow viewports and in square exports.
    const scale = Math.min(3.85 / size.y, (4.1 * Math.min(1, aspect)) / size.x);
  ...
  useFrame((_, delta) => {
    ...
    const target = viewRotation(view);
  ...
  const onPointerDown = (event: ThreeEvent<PointerEvent>) => {
    ...
    const zone = zoneMask
      ...
      : zoneFromAtlasUv(event.uv.x, event.uv.y, currentGarment().manifest);
    if (!zone) return;
    setSelectedZone(zone);
    const state = useEditorStore.getState();
    const selected = state.document.layers.find(
    ...
    if (
      ...
      const frame = getLayerFrame(
        ...
        rect,
        selected,
        imageAspectFor(selected, assets),
      );
      const point = pointInAtlasRect(zone, event.uv, zoneMask);
      const handle = hitTestLayerHandle(
        frame,
        point.x,
        point.y,
      ...
      if (handle) {
        const anchor = rotatedPoint(
          frame,
          oppositeHandle(handle),
          selected.transform.rotation,
        );
        resizeGesture.current = {
          ...
          frame,
          anchor,
          corner: rotatedPoint(frame, handle, selected.transform.rotation),
    ...
    const point = localPointFromAtlasUv(
      zone,
      event.uv.x,
      event.uv.y,
      zoneMask?.rects?.[zone],
  ...
  const onPointerMove = (event: ThreeEvent<PointerEvent>) => {
    ...
    if (resizeGesture.current) {
      ...
      event.stopPropagation();
      const point = pointInAtlasRect(
        resizeGesture.current.zone,
        event.uv,
        zoneMask,
      );
      const next = resizeFromPoint(resizeGesture.current, point);
      const rect = atlasRectFor(resizeGesture.current.zone, zoneMask);
      updateLayerResizeLive(
        resizeGesture.current.id,
        next.scale,
        (next.x - rect.x) / rect.width,
        (next.y - rect.y) / rect.height,
      );
      return;
    }
    const state = useEditorStore.getState();
    ...
    setResizeCursor(
      ...
      interactionMode === 'move' &&
        hoverZone &&
        selected?.zone === hoverZone &&
        !selected.locked
        ? layerHandleAtPointer(
            hoverZone,
            event.uv,
            selected,
            assets,
    ...
    const layer = useEditorStore
      .getState()
      .document.layers.find((item) => item.id === dragLayer.current);
    if (!layer) return;
    event.stopPropagation();
    const point = localPointFromAtlasUv(
      layer.zone,
...
function CaptureBridge({
  ...
  useEffect(() => {
    const modelId = currentGarment().id;
    const capture = async () => {
      const state = useEditorStore.getState();
      if (state.document.modelId !== modelId || state.stageStatus !== 'ready')
        throw new Error('El visor no está listo.');
      if (!groupRef.current)
        throw new Error('El modelo aún se está preparando.');
      ...
      try {
        ...
        for (const view of ['front', 'back', 'left', 'right'] as ViewId[]) {
          if (
            !groupRef.current ||
            useEditorStore.getState().document.modelId !== modelId
          )
            throw new Error('La prenda cambió durante la captura.');
          groupRef.current.rotation.y = viewRotation(view);
          gl.render(scene, camera);
          captures[view] = await new Promise<Blob>((resolve, reject) =>
            gl.domElement.toBlob(
              (blob) =>
                blob
                  ? resolve(blob)
                  : reject(new Error('No se pudo crear una vista PNG.')),
              'image/png',
            ),
          );
        }
      } finally {
        if (groupRef.current) groupRef.current.rotation.y = originalRotation;
    ...
    return registerCapture(modelId, capture);
  }, [camera, gl, groupRef, invalidate, scene]);

  return null;
}

function WebGLWatcher({
  onLost,
  onRestored,
}: {
  onLost: () => void;
...
export function NewGarmentStage() {
  const garment = currentGarment();
  const exporting = useEditorStore((state) => state.exportStatus === 'working');
  const webgl2 = useMemo(
    () => Boolean(window.document.createElement('canvas').getContext('webgl2')),
    [],
  );
  ...
  const interactionMode = useEditorStore((state) => state.interactionMode);
  const setInteractionMode = useEditorStore(
    (state) => state.setInteractionMode,
  );
  const selectedZone = useEditorStore((state) => state.selectedZone);
  useEffect(() => {
    if (!webgl2 || contextLost)
      useEditorStore.getState().setStageStatus(garment.id, 'error');
  }, [webgl2, contextLost, garment]);
  ...
  return (
    ...
    <section
      ...
      role="application"
      tabIndex={0}
      className="editor-grid relative h-full min-h-0 overflow-hidden focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-sky-500"
      aria-label={`Visor 3D: ${garment.label}. Usa las flechas para mover la capa seleccionada.`}
    >
      <Canvas
        frameloop="demand"
        dpr={[1, 1.75]}
      ...
      >
        ...
        <WebGLWatcher
          ...
          onRestored={() => {
            useEditorStore.getState().setStageStatus(garment.id, 'loading');
      ...
      {progress.active ? (
        <output
          className="pointer-events-none absolute inset-0 z-20 grid place-items-center bg-background/76 backdrop-blur-sm"
          aria-live="polite"
        >
          <div className="w-56 rounded-2xl border bg-card p-4 shadow-lg">
            <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
              <span>Cargando {garment.label}</span>
              <span>{Math.round(progress.progress)}%</span>
            </div>
            <progress
              className="mt-2 block h-1.5 w-full overflow-hidden rounded-full accent-sky-500"
              aria-label="Carga del modelo 3D"
              max={100}
      ...
      {modelError ? (
        <output
          className="absolute left-1/2 top-12 z-20 flex -translate-x-1/2 items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900 shadow-lg"
          aria-live="polite"
        >
      ...
      {contextLost ? (
        <div
          className="absolute inset-0 z-30 grid place-items-center bg-background/80 p-6 backdrop-blur-sm"
          role="alertdialog"
          aria-modal="true"
          aria-labelledby="webgl-recovery-title"
        >
          <div className="max-w-sm rounded-2xl bg-card p-5 text-center shadow-xl">
            <Box className="mx-auto mb-2 size-8 text-amber-500" />
            <h2 id="webgl-recovery-title" className="font-heading font-bold">
              Reconectando el visor…
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Tu diseño está a salvo. Si no vuelve en unos segundos, recarga la
              página.
            </p>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-3 flex justify-center md:top-4">
        <div className="rounded-full border bg-card/82 px-3 py-1.5 text-xs font-medium text-muted-foreground shadow-sm backdrop-blur-md">
          Zona activa:{' '}
          <span className="text-foreground">{ZONE_LABELS[selectedZone]}</span>
        </div>
      </div>
      ...
      <StageToolbar>
        <Tooltip>
          ...
          <TooltipContent side="left">Restablecer vista</TooltipContent>
        </Tooltip>
        <Tooltip>
          <TooltipTrigger
            render={
      ...
      <div className="absolute bottom-4 left-4 hidden items-center gap-1 rounded-xl border border-sidebar-border bg-sidebar/90 p-1 text-sidebar-foreground shadow-lg backdrop-blur md:flex">
        <Button
          ...
          size="sm"
          className={cn(
            'h-9 text-xs text-white hover:bg-white/12 hover:text-white',
            interactionMode === 'move' && 'bg-sky-500 hover:bg-sky-500',
          )}
          onClick={() => setInteractionMode('move')}
        >
          <MousePointer2 /> Editar
        ...
        <Button
          ...
          className={cn(
            'h-9 text-xs text-white hover:bg-white/12 hover:text-white',
            interactionMode === 'rotate' && 'bg-sky-500 hover:bg-sky-500',
          )}
          onClick={() => setInteractionMode('rotate')}
        >
          <Rotate3D /> Girar
